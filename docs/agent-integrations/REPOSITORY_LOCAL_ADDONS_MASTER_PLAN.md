# Repository-Local Add-ons Master Plan — orca-strator

## Status

**PLANNING ONLY / IMPLEMENT IN A LATER SESSION**

Target repository: `quantdale/orca-strator`  
Planning branch: `plan/repo-local-addons-2026-08-28`

## Repository assessment

Windows-first autonomous development orchestrator using Node/TypeScript, Electron, Fastify, React/Vite, SQLite, Git and later Playwright-driven ChatGPT browser automation.

**Decision:** `RECOMMEND_DEV_DIAGNOSTICS_PLUS_CURRENT_DOCS`

Keep Orca's product contract harness-neutral. New MCPs are developer tools only and must not become required for an autonomous run.
...
**Why it fits:** Useful for loopback UI/Electron renderer diagnostics, performance and browser-side debugging during orchestrator development.

**Constraints:** Development-only; it must not become Orca's production Sol automation channel or bypass Orca's own Playwright contracts.

### 2. Context7 MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Current Electron, Fastify, React, Vite and Node APIs.

**Constraints:** Docs only.

## Explicitly not recommended

- GitHub MCP as a runtime dependency—Orca's durable Git protocol should remain explicit
- A second browser automation authority in production
- Global CLI-agent configuration changes
