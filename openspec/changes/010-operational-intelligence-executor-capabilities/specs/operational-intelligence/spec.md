...
- Scenario: Settings opens or a GET readiness request is made.
  - Then Orca SHALL perform at most STATIC checks and SHALL NOT silently issue a
    model request.
...
- Scenario: Kimi, Codex, generic CLI, or deterministic test CLI is configured.
  - Then each SHALL use the same capability contract; unknown optional features
    SHALL be reported as UNKNOWN/NOT_APPLICABLE rather than guessed.
- Scenario: A capability probe cannot check auth or model recognition without a
...
- Scenario: The campaign wall-clock ceiling crosses while an actor is active.
  - Then Change 009's `DRAINING`/natural completion behavior SHALL remain; the
