...
- [x] Q.7 Real multi-repository concurrency — **MACHINE-QUALIFIED** via `Q.WIN.3` (two repos, one assembled controller, concurrent child processes). Real `emergencyKill` isolation **proven by the non-skipped `real-runtime-controls.test.ts`**: kill A terminates only A's process tree → `RECOVERY_REQUIRED` with dispatch A not falsely consumed; sibling B stays alive, finishes naturally, result consumed, Sol handoff proceeds.
