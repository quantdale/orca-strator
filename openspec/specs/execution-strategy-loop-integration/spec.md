...
Compose the already-built `SINGLE_AGENT`, `SWARM`, and `DAG` execution
strategies into Orca's canonical autonomous campaign loop so exactly one
execution strategy owns each repository/campaign iteration, durably, with the
same actor-ownership, control, budget, recovery, and Git-truth guarantees as
the hardened V1 single-agent loop.
