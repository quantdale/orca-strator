# external-chrome-auth-bootstrap Specification

## Purpose

Separate HUMAN AUTHENTICATION from AUTOMATION for the dedicated ChatGPT
browser profile. Google OAuth rejects Playwright-managed browsers, so the
INTERACTIVE_SETUP login window is an ordinary installed Chrome child process
with no remote automation attached, while production automation reuses the
same dedicated profile through Playwright after the human completes sign-in.
No security-evasion technique is permitted anywhere in these flows.
