# Abortable Runtime Lifecycle Specification

## Purpose

Guarantee that controller startup, listen failure, signal interruption and shutdown cannot strand child processes, browser profiles, worktrees, timers, sockets, database ownership or the singleton lock. Construction is cancellable, partial construction has deterministic cleanup, and automated browser-profile reclaim is conservative about host ownership evidence.
