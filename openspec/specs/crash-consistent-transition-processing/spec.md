# Crash-Consistent Transition Processing Specification

## Purpose

Guarantee that no durable protocol source — dispatch, Sol control, executor completion or strategy completion — can be consumed without its required run-state transition being durably applied in the same transaction. External side effects move to a durable outbox with deterministic effect keys so replay after a crash is idempotent rather than duplicative.
