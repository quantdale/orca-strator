...
Make the strategy completion boundary authoritative and the integration path
concurrency-safe: a SWARM/DAG iteration is durably successful only when its
remote publication is pushed AND verified, remote main movement is classified
and reconciled safely before SHA-truthful manifests are published, DAG staging
lands on a strategy-owned serialized lineage, campaign controls are awaited and
acknowledged, and controller shutdown/restart sweeps leave no orphaned children,
false successes, or unrecoverable state.
