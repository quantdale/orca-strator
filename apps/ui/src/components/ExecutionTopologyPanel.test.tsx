...
describe("ExecutionTopologyPanel", () => {
  it("renders the real single-agent Sol handoff sequence", () => {
    ...
    expect(screen.getByTestId("topology-strategy").textContent).toBe(
      "SINGLE_AGENT",
    );
  ...
  it("renders actual DAG dependency and partial integration evidence", () => {
    ...
    expect(
      screen
        .getAllByTestId("topology-strategy")
        .find((element) => element.textContent === "DAG")?.textContent,
    ).toBe("DAG");
    expect(screen.getByText("Node b")).toBeTruthy();
    expect(
      screen.getByTestId("topology-dependencies-packet-packet-b"),
    ).toBeTruthy();
