...
export type WorkPacketStatus = "QUEUED" | "STARTING" | "RUNNING" | "WAITING_PERMISSION" | "RETRYING" | "COMPLETED" | "FAILED" | "BLOCKED" | "SKIPPED_DEPENDENCY" | "CANCELLED";
export type WorkPacketResultStatus = "COMPLETED" | "FAILED" | "BLOCKED" | "CANCELLED" | "SKIPPED" | "SKIPPED_DEPENDENCY" | "INTEGRATION_CONFLICT";
export type WorktreeLifecycleStatus = "ALLOCATED" | "ACTIVE" | "RELEASED" | "STALE" | "CLEANUP_REQUIRED" | "ORPHANED";
export type IntegrationStatus = "COMPLETED" | "PARTIAL" | "INTEGRATION_CONFLICT" | "BLOCKED";
...
const packetExecutorSchema = z.object({
...
export const workPacketSchema = z.object({
  ...
  verificationExpectations: z.array(z.string().trim().min(1).max(1000)).max(100),
  budget: z.object({
  ...
  status: z.enum(["QUEUED", "STARTING", "RUNNING", "WAITING_PERMISSION", "RETRYING", "COMPLETED", "FAILED", "BLOCKED", "SKIPPED_DEPENDENCY", "CANCELLED"]),
...
export const workPacketResultSchema = z.object({
  schemaVersion: z.literal(1),
  packetId: z.string().trim().min(1).max(200),
  campaignId: z.string().trim().min(1).max(200),
  runId: z.string().trim().min(1).max(200),
  iteration: z.number().int().positive(),
  status: z.enum(["COMPLETED", "FAILED", "BLOCKED", "CANCELLED", "SKIPPED", "SKIPPED_DEPENDENCY", "INTEGRATION_CONFLICT"]),
